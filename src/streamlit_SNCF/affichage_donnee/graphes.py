...
def listeRetardsMoyensFrance(df: pd.Dataframe) -> None:
    """
    Affiche la liste complète des retards moyens au départ de toutes les gares de France
    """
...
def histRetardVilleDepart(df: pd.DataFrame) -> None:
    """
    Affiche l'histogramme des retards moyens au départ d'une ville de départ sélectionnée
    """
    
    st.subheader("📋 Histogrammes retards au départ par ville de départ")
    
    gares=sorted(list(df["GareDepart"].unique()))
    ville_depart = st.selectbox("Ville de départ :", gares,)
